;(function ($, window, document, undefined) {
	...
	HwGrid.prototype = {
		...
		_getText : function (content) {
			...
			str += '<input type="text" id="'+(content.id == undefined ? "" : content.id)+'"  name="' + content.name + '" value="'+(content.value == undefined ? '' : content.value)+'" class="form-control">';
			str += '</div>';
			return str;
				...
				$.each(that._data.list,function (index, content) {
					...
					if (that._actionCloums != undefined) {
						...
						$.each(that._actionCloums,function (index, content) {
							...
							var a = $('<a ' + cls+ ' id="'+ content.id+ '">'+ text+ '</a>');
							a.click(function (e) {
